...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetchGetData(
        'https://damp-castle-86239-1b70ee448fbd.herokuapp.com/decoapi/genericchat/', // API 端点 URL / API endpoint URL
        {
        ...
        if (!data) {
            container.innerHTML = '<p class="text-danger">Unable to load genericchat.</p>'; // 显示错误消息 / Display error message
            return;
        }
        ...
        data.forEach(member => {
            ...
            card.innerHTML = `
        <div class="card-body">
          <h5 class="card-title">${member.person_name}</h5>       <!-- 成员姓名 / Member name -->
          <h5 class="card-title">${member.chat_date_time}</h5>       <!-- 成员姓名 / Member name -->
          <p class="card-text">${member.chat_post_title || 'No message provided.'}</p> <!-- 成员留言或默认提示 / Member message or default note -->
          <p class="card-text">${member.chat_post_content || 'No message provided.'}</p> <!-- 成员留言或默认提示 / Member message or default note -->
        </div>
      `;
